// modified version of Hayd's JSZip module.
// optimised for DownHost app.

import _zip from 'https://jspm.dev/jszip';
 ...
 * @return Returns promise
 */
export async function zipDir( dir: string ): Promise<JSZip> {
  const z = new JSZip();
  try {
    for await (const entry of Deno.readDir(dir)) {
        if (entry.isDirectory || entry.isSymlink) continue; // we'll ignore subdirectory.
        const contents = await Deno.readFile(join(dir,entry.name));
        z.addFile(entry.name,contents)
    }
  }
  catch(e) {
      console.log(e);
...
export class JSZip {
  ...
  // we should assert the type (we want it to be a _zip) ?
  constructor(z?: any) {
    if (z === undefined) {
      // @ts-ignores
      this._z = new _zip();
